...
def get_filepath(opt):
    ...
    for name in file_name:
        filepaths.append(os.path.join(opt['datapath'], opt['dataset'],name + '.tsv'))
    return filepaths
...
def load_file(filename,onehot=True):
    ...
    with open(filename, newline='', encoding='utf-8') as f:
        read = csv.reader(f, delimiter='\t',quoting=csv.QUOTE_NONE,quotechar='|')
        next(read)
        for i,line in enumerate(read):
            label, id1, id2, d1, d2 = line#.rstrip().split('\t')
            ID1.append(id1)
            ID2.append(id2)
            D1.append(d1)
            D2.append(d2)
